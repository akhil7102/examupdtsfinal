...
export interface Post {
  ...
  youtube_links: string[]; // DEPRECATED
  external_link?: string; // New: Optional external link
  timetable_link?: string; // New: Optional timetable link
  exam_dates?: { subject: string; date: string }[]; // New: Array of subject-date pairs
  date: string;
  views: number;
  author?: string;
  trending?: boolean;
...
export interface Result {
  ...
  status: string;
  link: string;
  server1_link?: string; // New: Server 1 link
  server2_link?: string; // New: Server 2 link
  pdf_file?: string;
